...
module.exports = function SlackInfo(clients) {
  ...
  const changes = {
    files$: Observable.merge(FILE_EVENTS.map(rtmEventStream))
      .mergeAll()
      .startWith(true),
    channels$: Observable.merge(CHANNEL_EVENTS.map(rtmEventStream))
      .mergeAll()
      .startWith(true),
    users$: Observable.merge(USER_EVENTS.map(rtmEventStream))
      .mergeAll()
      .startWith(true)
  };
  ...
  const records$ = Observable.combineLatest(files$, channels$, users$, function(
    files,
    channels,
    users
  ) {
